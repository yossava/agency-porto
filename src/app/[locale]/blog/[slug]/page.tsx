...
import { getBlogPostBySlug, formatDate, getAllBlogPosts } from '@/lib/content';
        ...
        >
          <div className="glass rounded-3xl p-8 md:p-12">
            ...
            <div className="text-gray-300 leading-relaxed space-y-6">
              <p className="text-xl text-gray-400 mb-8">{post.excerpt[currentLocale]}</p>

              <div className="whitespace-pre-wrap">{post.content[currentLocale]}</div>

              {/* Placeholder content for demo */}
              {post.content[currentLocale].length < 100 && (
                <>
                  <p>
              ...
              {relatedPosts.map((relatedPost) => (
                ...
                >
                  <div className="text-sm text-blue-400 mb-2">
                    {relatedPost.category[currentLocale]}
                  </div>
                  <h3 className="text-lg font-bold mb-2 group-hover:gradient-text transition-all">
                    {relatedPost.title[currentLocale]}
                  </h3>
                  <p className="text-sm text-gray-400 line-clamp-2 mb-3">
                    {relatedPost.excerpt[currentLocale]}
                  </p>
                  <div className="text-xs text-gray-500">
                    {formatDate(relatedPost.date, currentLocale)} •{' '}
